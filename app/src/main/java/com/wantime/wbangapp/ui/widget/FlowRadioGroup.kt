...
class FlowRadioGroup : RadioGroup {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        for (i in 0 until count) {
            ...
            if (lineWidth + deltaX + paddingLeft + paddingRight > widthSize) { //如果折行,height增加
                ...
                Log.v(
                    TAG,
                    "maxHeight:$totalHeight---maxWidth:$maxWidth"
                )
            } else {
              //不换行，累加宽度
                lineWidth += deltaX
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        for (i in 0 until count) {
            ...
            if (preLeft + params.leftMargin + child.measuredWidth + params.rightMargin + paddingRight > r - l) { //重置
                preLeft = paddingLeft
    ...
    companion object {
        private const val TAG = "RadioGroupEx"
    }
}